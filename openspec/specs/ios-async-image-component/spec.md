# ios-async-image-component Specification

## Purpose

定義 iOS 端遠端圖片載入元件 `RemoteAsyncImage`，統一處理 TMDB 相對圖片路徑補 host、Kingfisher 快取，以及 Loading／Success／Error 狀態呈現。
...
- **THEN** iOS 圖片元件 SHALL 在呼叫端提供的容器邊界內以 Error 內容取代圖片，且不得改變容器尺寸或保留 Error 覆蓋圖片的外層堆疊
