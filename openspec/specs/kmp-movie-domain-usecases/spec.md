...
- **THEN** `UserDataRepository.setConfiguration(configuration)` 被呼叫，且回傳的 `Flow` emit `Result.success(configuration)`
...
- **THEN** 回傳的 `Flow` emit `Result.success(cachedConfiguration)`
