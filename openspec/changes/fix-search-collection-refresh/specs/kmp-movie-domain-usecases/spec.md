...
`shared/domain` 的 `commonMain` SHALL 提供 `GetSearchMovieListUseCase`，合併 `MovieRepository.getMovieSearchPager(query)` 的分頁資料與 `getCollectedMovieIds()`，並將每筆搜尋結果的 `isCollect` 更新為目前實際收藏狀態。UseCase MUST 接收呼叫端提供的 `CoroutineScope` 以快取搜尋 Pager。

#### Scenario: 搜尋結果中已收藏電影標記為 true

- **WHEN** 呼叫 `GetSearchMovieListUseCase(query, scope)` 並收集資料，且某筆搜尋電影 id 存在於 `getCollectedMovieIds()` 結果
