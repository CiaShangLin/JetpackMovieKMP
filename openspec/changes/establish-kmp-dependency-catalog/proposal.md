...
- **BREAKING**：Room 由 `androidx.room` 2.6.1 規劃切換至 KMP 導向的穩定版 `androidx.room3` 3.0.0，並搭配 Room 3 plugin、KSP 與 bundled SQLite；升版需明確驗證 package、Coroutine API、schema 與產生碼。
...
- 依賴/API 影響：Retrofit/OkHttp 改為 Ktor，Hilt 改為 Koin，Room 2 package 改為 Room 3 package；既有攔截器、DI modules、DAO/database 與測試日後需逐模組改寫。
