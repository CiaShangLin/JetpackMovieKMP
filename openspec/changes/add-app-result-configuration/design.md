...
- 把 `MovieRepository.getConfiguration()` 與 `GetConfigurationUseCase` 這條「iOS 端已有實際消費需求」
  的路徑，從 `kotlin.Result<T>` 遷移到 `AppResult<T>`，驗證型別設計在真實路徑上可行。
...
Repository／UseCase 職責劃分，只是把 UseCase／Repository 對外回傳的「成功/失敗容器」型別換掉，
屬於既有 MVVM + Repository/UseCase 模式下的型別替換，非架構偏離。
...
2. **`AppError` 本次只定義 `Network`／`Unknown` 兩種**
   - 選擇：`sealed interface AppError { data class Network(val exception: NetworkException) :
     AppError; data object Unknown : AppError }`
   - 理由：`getConfiguration()` 的失敗來源目前 100% 是 `MovieRepositoryImpl` 對 `NetworkResponse.error`
     ...
     分類在這條路徑上沒有任何實際案例可驗證，先定義只會是憑空猜測的介面設計，且 `sealed interface`
...
6. **範圍邊界：`shared/app` iosMain 與 `iosApp` Swift 不在本次實作任務內**
   - 理由：依使用者明確分工——iOS 端（含 `shared/app` iosMain 的 Kotlin 程式碼）由使用者自行處理，
     Android／共用（`shared/common`、`shared/data`、`shared/domain`、`androidApp`）由本次 change
     實作。spec delta（`ios-koin-bridge`、`ios-splash-screen`）仍會同步更新，作為使用者對齊實作的
     依據，但程式碼改動不列入 `tasks.md`。

## Risks / Trade-offs

- **[Risk]** `MovieRepository`／UseCase 層此後會同時存在 `AppResult<T>`（`getConfiguration`）與
  `kotlin.Result<T>`（其餘 4 個方法）兩種回傳型別，呼叫端需要分別處理，容易混淆。
  → **Mitigation**：在 `MovieRepository` 介面對應方法加上 KDoc 註記型別差異；本次不強行一次遷移
  全部方法以控制改動範圍，待評估後續 change 排程時全部遷移完成後即消除此不一致。
...
- **[Risk]** 之後要新增 `AppError.Database`／`AppError.LocalStorage` 時，所有既有對 `AppError` 做
  ...
  → **Mitigation**：這是 `sealed interface` 的設計目的所在——編譯器會在新增 case 時，對所有未窮舉
  的 `when` 主動報錯，不會是靜默的執行期問題。

## Migration Plan

1. `shared/common`：新增 `AppResult`／`AppError`，補單元測試（成功/失敗建構、`sealed interface`
   窮舉行為）。
2. `shared/data`：`MovieRepository.getConfiguration()` 介面簽名與 `MovieRepositoryImpl` 實作改為
   回傳 `Flow<AppResult<ConfigurationBean>>`；更新既有測試（`MovieRepositoryImplTest` 等）。
3. `shared/domain`：`GetConfigurationUseCase` 改為回傳 `Flow<AppResult<ConfigurationBean>>`，
   fallback 邏輯改寫為對應 `AppResult` 的 `when` 分支；更新既有測試。
4. `androidApp`：`MainViewModel`／`MainUiState` 改為消費 `AppResult`；執行
   `:androidApp:assembleDebug` 驗證編譯與啟動皆正常（Loading／Error／Success 三態行為不變）。
5. 更新 `common-kernel`、`kmp-movie-data-repository`、`kmp-movie-domain-usecases`、
   `ios-koin-bridge`、`ios-splash-screen` 五份 spec delta。
