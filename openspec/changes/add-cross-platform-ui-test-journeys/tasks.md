...
- [ ] 1.1 確認 Android 與 iOS UI journey 皆維持 production Koin 組裝，且不新增 UI-test launch mode。
- [ ] 1.2 定義使用正式資料來源時的資料不足、不適用與結果記錄規則。
- [ ] 1.3 實作每次旅程前的收藏、歷史、主題及語言設定重置；補齊受影響平台測試。
- [ ] 1.4 確認本 change 不修改 `shared/app` 的 iOS 匯出邊界；在 macOS 執行對應 iOS simulator 測試。

## 2. Android selector 與 UI 自動化支援

- [ ] 2.1 在 `androidApp` 為頂層導覽與測試啟動模式補齊穩定 selector，並補相關導覽／狀態單元測試。
- [ ] 2.2 在 `feature/home` 為首頁類型 Tab、Pager 與電影卡補齊 Compose semantics／`testTag`；補 ViewModel 或狀態邏輯測試。
...
- [ ] 3.2 建立 iOS UI test target 的 production 資料啟動與本機狀態重置流程，記錄資料不足時的不適用結果。
...
- [x] 4.1 在 `docs/uitests` 建立 UC-01 至 UC-13（排除 UC-14）的 `journey.xml`，記錄已確認的操作與驗收條件。
- [x] 4.2 建立 UC-15A 首頁分頁與 UC-15B 搜尋分頁的獨立 `journey.xml`，僅驗證第二頁出現新電影卡，不驗證 loading footer。
