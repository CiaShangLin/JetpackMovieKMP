...
### Requirement: common 提供共用 CoroutineScope 的 Koin module

`shared/commonMain` MUST 在 `common` package 提供 `commonModule()`（Koin module function），綁定單一 `CoroutineScope` single；其他 module（例如 `datastoreModule`）MUST NOT 自行重複定義等價的 `CoroutineScope` binding。
