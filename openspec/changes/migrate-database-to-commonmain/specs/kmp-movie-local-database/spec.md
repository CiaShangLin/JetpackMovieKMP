...
`shared` MUST 提供 Koin database module，接受已建立好的 `RoomDatabase.Builder<AppDatabase>`，並可 resolve `AppDatabase`、`MovieCollectDao`、`MovieHistoryDao`。

#### Scenario: database module 可解析 AppDatabase

- **WHEN** Koin 安裝以有效 `RoomDatabase.Builder<AppDatabase>` 建立的 database module
