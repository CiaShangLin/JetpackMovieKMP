...
# 執行較完整的 Gradle 驗證（含 ktlint；不含 iOS Swift lint）
./gradlew check
```
...
.\gradlew.bat :shared:data:testAndroidHostTest
.\gradlew.bat :shared:data:iosSimulatorArm64Test
...
- 優先沿用專案既有模式（Repository / UseCase / Koin module / Mapper），再考慮新增結構，偏離時需說明原因。
- 可跨 Android 與 iOS 共用的 business logic 與 UI logic 優先放在 `shared`。
- Android-only 行為放在 `androidApp`、`core/*` 或對應模組的 `androidMain`。
- iOS-only 行為放在 `iosApp` 或對應模組的 `iosMain`。
- Compose Multiplatform 相關實作遵循既有 `commonMain` 慣例。
- 涉及資料庫 schema 變更時需考慮 Room migration 策略（schema 輸出在 `shared/database/schemas`）。
- 修改範圍應聚焦在使用者要求的行為，避免無關重構。
- 不要提交 secrets 或機器本機專屬設定。
- 除非使用者明確要求，`local.properties`、`key.properties`、簽章檔、產生的 build output 都視為非原始碼產物。
...
- proposal 的 Impact 段落須列出受影響模組（例如 `shared/data`、`core/ui`）
- specs 的 Requirement / Scenario 以繁體中文撰寫
- tasks 以模組為單位分組，涉及 UI 的 task 需包含對應單元測試
